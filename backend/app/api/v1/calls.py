...
async def initiate_zadarma_call(
    ...
    result = await zadarma.initiate_call(body.from_number, body.to_number)
    return result


@router.post("/zadarma/webhook", summary="Webhook статуса звонка от Zadarma")
async def zadarma_webhook(payload: dict, db: AsyncSession = Depends(get_db)):
    logger.info("Zadarma webhook received: %s", payload)
    call_status = payload.get("call_status")
    phone = payload.get("caller_id") or payload.get("called_did", "")

    if phone and call_status in ("cancel", "busy", "failed"):
        result = await db.execute(
            select(Lead).where(Lead.phone == phone, Lead.status == "calling")
        )
        lead = result.scalar_one_or_none()
        if lead:
            task_result = await db.execute(
                select(CallTask).where(CallTask.lead_id == lead.id, CallTask.completed == False)
            )
            ...
            if task:
                ...
                if task.attempts >= 3:
                    ...
                    task.next_call_at = datetime.now(timezone.utc) + timedelta(hours=3)
            await db.commit()
